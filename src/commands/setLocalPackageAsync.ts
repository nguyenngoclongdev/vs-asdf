...
export const setLocalPackageAsync = async (config: ExtensionConfiguration, treeItem: TreeItem): Promise<void> => {
    ...
    try {
        ...
        window.withProgress(
            ...
            async (progress) => {
                progress.report({ message: `Set ${packageName}@${packageVersion} as the local version...` });
                const command = new CommandBuilder(bin, asdf.setLocalPackage)
                    .replace('<name>', packageName)
                    .replace('<version>', packageVersion)
                    .build();
                const { stderr } = cmd.runSync(command);
                if (stderr) {
                    const message = `Unable to set ${packageName}@${packageVersion} as the local version.`;
                ...
                window.showInformationMessage(
                    `${packageName}@${packageVersion} has been successfully set as the local version.`
                );
        ...
        showErrorMessageWithDetail(`Failed to set ${packageName}@${packageVersion} as the local version.`, error);
    }
};
