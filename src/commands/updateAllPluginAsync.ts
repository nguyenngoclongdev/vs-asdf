...
export const updateAllPluginAsync = async (config: ExtensionConfiguration): Promise<void> => {
    ...
    try {
        window.withProgress(
            ...
            async (progress) => {
                progress.report({ message: 'Update all plugins...' });
                const command = new CommandBuilder(bin, asdf.updateAllPlugin).build();
                const { stderr } = cmd.runSync(command);
                if (stderr) {
                    const message = `Unable to update all plugins.`;
                ...
                window.showInformationMessage(`All plugins have been successfully updated.`);
        ...
        showErrorMessageWithDetail(`Failed to update all plugins.`, error);
    }
};
